	...
	const rawTimezone =
		typeof document === "undefined"
			...
			: getComputedStyle(document.documentElement)
					.getPropertyValue("--ryu-timezone")
					.trim();
	const rawLocale =
		typeof document === "undefined"
			? "system"
			: getComputedStyle(document.documentElement)
					.getPropertyValue("--ryu-locale")
					.trim();
	try {
		return new Intl.DateTimeFormat(
			rawLocale && rawLocale !== "system" ? rawLocale : undefined,
			{
				dateStyle: "medium",
				timeStyle: "medium",
				timeZone:
					rawTimezone && rawTimezone !== "system" ? rawTimezone : undefined,
			}
		).format(date);
	} catch {
		return new Intl.DateTimeFormat(
